...
ticker2 = input("Enter Ticker 2: ").upper()


try:
    data1 = yf.download(ticker1, period="6mo", auto_adjust=True)["Close"]
    data2 = yf.download(ticker2, period="6mo", auto_adjust=True)["Close"]
...
if data1.empty:
    ...
    sys.exit()
if data2.empty:
    print(f"No data found for {ticker2}")
    sys.exit()
...
plt.xlabel(ticker2)
plt.ylabel(ticker1)
plt.title(f"{ticker1} vs {ticker2} Linear Regression")
plt.legend()
plt.show()
